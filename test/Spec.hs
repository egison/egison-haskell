...
pmap f xs = matchAll xs (list something)
             $ [mc| join Wildcard (cons $x Wildcard) => f x |] .*. PNil

pmConcat :: [[a]] -> [a]
pmConcat xss = matchAll xss (multiset (multiset something))
               $ [mc| cons (cons $x Wildcard) Wildcard => x |] .*. PNil
...
spec = do
  describe "match and matchAll" $ do
    it "list cons pattern" $
      match [1,2,5,9,4] (list integer) ([mc| cons $x $xs => (x, xs) |] .*. PNil)
      `shouldBe` (1, [2,5,9,4])

    it "multiset cons pattern" $
      matchAll [1,2,5,9,4] (multiset integer) ([mc| cons $x $xs => (x, xs) |] .*. PNil)
    ...
    it "pattern matching with infinitely many results" $
      take 10 (matchAll primes (list integer)
                ([mc| join Wildcard (cons $p (join Wildcard (cons #(p+6) Wildcard))) => (p, p+6) |] .*. PNil))
    ...
    it "Value patterns, and-patterns, or-patterns, and not-patterns" $
      matchAll [1,2,5,9,4] (multiset integer)
        ([mc| cons (AndPat (NotPat #5) $x) (cons (AndPat (OrPat #5 #2) $y) $xs) => (x, y, xs) |] .*. PNil)
    ...
    it "Predicate patterns" $
      matchAll [1..10] (multiset integer)
        ([mc| cons (AndPat (PredicatePat (\x -> mod x 2 == 0)) $x) Wildcard => x |] .*. PNil)
      `shouldBe` [2,4,6,8,10]
