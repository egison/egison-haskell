...
matchAll :: a -> Matcher m -> [(Pattern a (Matcher m) (HList vs), (HList vs) -> b)] -> [b]
matchAll tgt matcher =
  foldr (\(pat, f) matches ->
    case runPattern pat tgt of
      Nothing -> []
      Just vs -> [f vs]) []
...
-- processMState (MState (MAtom pat tgt:atoms) rs) = [MState (runPattern pat tgt:atoms) rs]
